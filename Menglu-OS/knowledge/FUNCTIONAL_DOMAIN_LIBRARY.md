# Functional Domain Library

## Purpose

This file defines the reusable functional domains used by Menglu OS when organising evidence, drafting advocacy materials, or checking whether a new statement duplicates existing information.

This is an index structure only. Do not store private evidence or detailed personal records in this public repository.

## Core rule

Functional evidence should describe what happens in real life, what support is required, what goes wrong without support, and what the consequences are.

## Functional domains

### Daily living

- Preparing food
- Eating and nutrition
- Hydration
- Medication management
- Washing and bathing
- Dressing and undressing
- Personal care routines
- Oral care, dental care, skin care, and product tolerance

### Communication and information

- Verbal communication
- Written communication
- Reading and understanding information
- Easy Read and simplified information needs
- Using assistive communication tools
- Appointment communication
- Retaining and acting on information

### Executive functioning and administration

- Starting tasks
- Sequencing tasks
- Prioritising
- Deadlines
- Follow-up actions
- Forms and correspondence
- Service coordination
- Decision-making
- Monitoring ongoing responsibilities

### Finance and safeguarding

- Budgeting decisions
- Understanding bills and bank information
- Identifying unusual transactions
- Financial risk monitoring
- Scam and exploitation vulnerability
- Appointee or structured financial support evidence

### Mobility and travel

- Planning journeys
- Following journeys
- Public transport barriers
- Driving safety
- Need for accompaniment
- Appointment travel burden
- Post-travel recovery

For accessible travel planning, also index:

- destination suitability
- travel method comparison
- accommodation accessibility
- step-free access and lift access
- queueing and waiting demands
- walking and standing demands
- sensory load during travel
- communication demand during travel
- food, hydration, medication, and allergy planning
- luggage handling and physical strain
- travel assistance requirements
- quiet spaces and reduced-stimulation routes
- support-person capacity
- recovery before and after travel
- backup plans and cancellation planning

Use these categories as an index only. Do not store live booking details, private travel records, personal identifiers, or detailed medical evidence in this public repository.

### Social engagement and psychological access

- Engaging with other people
- Group conversations
- Unstructured social interaction
- Unexpected approaches
- Shutdowns and meltdowns
- Psychological distress from interaction
- Counselling or therapy accessibility

### Sensory environment

- Sound
- Light and visual clutter
- Smell and chemical triggers
- Touch and clothing tolerance
- Food texture and oral sensitivity
- Deep pressure and regulation
- Low-sensory environment needs

### Health interaction and recovery

- POTS and orthostatic intolerance
- Chronic fatigue and post-exertional malaise
- Autistic burnout
- Hypermobility and pain
- Allergic rhinitis and environmental triggers
- Appointment recovery cost
- Flare-up impact
- Menstrual phase or premenstrual phase as a functional state modifier
- Appetite, hydration, dizziness, sensory tolerance, cognitive capacity, and recovery changes during cyclical health changes

### Functional state modifiers

Use this category when the issue is not a separate task domain, but a factor that changes reliable functioning across several domains.

Examples include:

- menstrual or premenstrual phase
- post-exertional malaise
- POTS flare or orthostatic symptom increase
- sensory overload or shutdown risk
- pain flare
- poor sleep
- illness or allergy flare
- recent high-demand appointment, travel, or administrative task

State modifier evidence should describe the practical effect, for example:

- whether leaving the house becomes unsafe or unrecoverable
- whether communication capacity reduces
- whether decisions or forms become unreliable
- whether appetite, hydration, or medication routines need support
- whether sensory tolerance narrows
- whether recovery time increases

Do not store private symptom diaries, cycle dates, medical identifiers, or detailed personal records in this public repository.

## Evidence quality checklist

For each domain, prefer examples that show:

1. The task or situation.
2. The barrier.
3. Support required.
4. What happens without support.
5. Delayed consequences.
6. Whether the issue happens reliably, repeatedly, safely, to an acceptable standard, and within a reasonable time.

## Real-world evidence rule

When available, use documented real-world incidents to support functional descriptions.

Examples of evidence types include:

- professional reports
- appointment outcomes
- written correspondence
- service-access barriers
- financial or administrative incidents
- assistive equipment records
- support sessions
- diaries or structured statements

Do not store the private details here. Store only the domain category and non-sensitive index reference.
